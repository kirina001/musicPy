...
if __name__ == '__main__':
    ...
    # 为空，则使用默认的
    if casepath == '':
        casepath = path + '/lib/cases/'+casename+'用例.xls'
        resultpath = path + '/lib/results/result-'+casename+'用例.xls'

        # casepath = path + '/lib/cases/SOAP接口用例.xls'
        # resultpath = path + '/lib/results/result-SOAP接口用例.xls'
    else:
        # 如果是绝对路径，就使用绝对路径
        if casepath.find(':') >= 0:
            # 获取用例文件名
